...
export default {
  ...
  state: {
    api: "cms",
    route: "api/cms/seo/text/%domain%/%lang%",
    batchObjectName: "CmsApiCmsSeoText%domain%%lang%",
    data: null,
    isDataLoading: false,
  ...
  mutations: {
    setIsDataLoading(state, payload = false) {
      ...
      if (window.debugLevel > 10) {
        console.debug("cmsSeoText/setDataLoading", state.isDataLoading);
      }
    },
    setData(state, payload) {
      if (!payload || !payload.data)
        throw new Error("cmsSeoText/setData needs to payload.data");
      state.data = payload.data;
      if (window.debugLevel > 50) {
        console.debug("cmsSeoText/setData data", state.data);
      }
    }
  },
  actions: {
    fetchData({ state, rootState, commit }, payload = {}) {
      ...
      if (window.debugLevel > 10) {
        console.debug("cmsSeoText/fetchData", payload, state.data);
      }
      // Возвращаем промис если данные уже грузятся
      if (state.isDataLoading) {
        if (window.debugLevel > 10) {
          console.debug("cmsSeoText/fetchData already in progress...");
        }
      ...
      this.dataPromise = new Promise((resolve, reject) => {
        ...
        if (
          ...
          if (window.debugLevel > 10) {
            console.debug(
              "cmsSeoText/fetchData loaded from CACHE",
              state.isDataLoaded,
              state.data
            );
          }
        ...
        let requestUrl =
          ...
          this._vm.$uuid.v1();
        if (window.debugLevel > 10) {
          console.debug("cmsSeoText/fetchData requestUrl", requestUrl);
        }
        ...
        axios
          ...
          .then(response => {
            if (!response.data) {
              throw new Error("cmsSeoText/fetchData response has no data");
            }
            if (window.debugLevel > 10) {
              console.debug("cmsSeoText/fetchData response", response);
            }
            state.isDataLoaded = new Date();
            commit("setData", response.data);
            if (window.debugLevel > 10) {
              console.debug("cmsSeoText/fetchData loaded from API", state.data);
            }
            resolve(state.data);
          })
          .catch(error => {
